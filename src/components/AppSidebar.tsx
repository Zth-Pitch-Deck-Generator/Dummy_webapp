...
export function AppSidebar({ currentStep, onStepChange, projectData, qaDone }: AppSidebarProps) {
  const steps = [
    ...
    { id: 'outline', title: 'Outline', icon: FileText, completed: false, disabled: !qaDone },
    { id: 'preview', title: 'Deck Preview', icon: Eye, completed: false, disabled: true },
  ];

  // A modern, clean, and professional design inspired by zth.co.in
  return (
                  ...
                  <SidebarMenuItem key={step.id}>
                    <SidebarMenuButton
                      ...
                      className={`
                        ...
                        ${isActive 
                          ? 'bg-[hsl(214,100%,50%)] text-white shadow-md hover:shadow-lg hover:bg-[hsla(214,100%,78%,1.00)] hover:text-gray-900' // Active state now has a lighter background and darker text on hover
                          : step.disabled 
                            ? 'text-gray-400 cursor-not-allowed' 
                            : 'text-gray-500 hover:bg-white hover:shadow-md hover:text-gray-900' // Inactive items lift with a shadow
                        }
                      `}
                      disabled={step.disabled}
                    >
                      {/* Icon color now also changes on hover for the active item */}
