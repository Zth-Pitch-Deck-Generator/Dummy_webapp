...
const handleOutline: RequestHandler = async (req, res) => {
  // This top-level try...catch block remains to prevent the server from crashing due to any unexpected errors.
  try {
    ...
    // This robust data validation remains to prevent the 'cannot read properties of undefined' crash.
    if (!project || !Array.isArray(project.qa_sessions) || project.qa_sessions.length === 0) {
      console.log(`No QA session found for projectId: ${projectId}`);
      res.status(404).json({ error: "Project or Q&A session missing" });
      return;
    }

    const qaSession = project.qa_sessions[0];
    if (!qaSession || !qaSession.transcript) {
      console.log(`Transcript is missing from QA session for projectId: ${projectId}`);
      res.status(404).json({ error: "Transcript is missing from the Q&A session." });
      return;
    }

    const slideCount = project.slide_count || 10;
    const transcript = qaSession.transcript as { role: string; content: string }[];
    const transcriptTxt = transcript
      .map((m: any) => `${m.role === "user" ? "Founder" : "Analyst"}: ${m.content}`)
      .join("\n");

    // --- CHANGE REVERTED: The AI prompt now asks for an "exactly" matching slide count again. ---
    const prompt = `
...
Transcript:"""${transcriptTxt}"""`;
    ...
    // --- CHANGE REVERTED: The Zod schema now enforces a strict length check again. ---
    ...
    res.json({ id: upserted.id, outline });
      ...
      console.error("An unexpected error occurred in handleOutline:", err);
...
Outline:${JSON.stringify(outlineRow.outline_json)}`;
    ...
    console.error("An unexpected error occurred in handleEval:", err);
    res.status(500).json({ error: "An internal server error occurred." });
  }
};
